...
def insert_rows_into_index(psql_client, psql_cursor, indexed_rows):
    dataText = ','.join(
        psql_cursor.mogrify('(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', row).decode('utf-8') for row in indexed_rows)

    response = psql_cursor.execute(
        'INSERT INTO {0} VALUES {1} ON CONFLICT DO NOTHING;'.format(PSQL_TABLE_NAME, dataText))
...
def test_if_table_exists(psql_cursor, table_name):
    ...
    return postgresql_cursor.fetchone()[0]
